...
def _best_word_segment_distance(
    ...
    for i in range(n):
        ...
        for j in range(j_min, j_max):
            ...
            score = 1.0 - (dist / max(target_len, 1))
            if score > best_score or (score == best_score and len(segment) < len(best_segment)):
                best_score = score
                best_dist = dist
                best_segment = segment
                if best_dist == 0:
...
def _best_sequence_for_line(
    ...
    for cut in range(len(words), 0, -1):
        ...
        dist, segment = _best_word_segment_distance(trimmed, target_words, length_tolerance)
        score = 1.0 - (dist / max(len(trimmed), 1))
        if score > best_score:
            best_score = score
            best_segment = segment
            best_len = len(trimmed)
        # stop early if dropping more words doesn't improve score
        if cut < len(words) and score < best_score:
            break
...
def _draw_best_sequences(
    ...
    scored = []
    for line in lines:
        line_text = line.get("text_words", "").strip()
        if not line_text:
            continue
        score, _ = _best_sequence_for_line(line_text, target_words)
        bbox = line.get("bbox")
        if not bbox:
            continue
        scored.append((score, len(line_text), bbox))
    if not scored:
        return
    scored.sort(key=lambda item: (item[0], item[1]), reverse=True)
    draw = ImageDraw.Draw(image)
    for score, _, bbox in scored[:top_k]:
        draw.rectangle(
...
def ocr_image(path: Path, lang: str) -> Dict[str, Any]:
    ...
    text = "\n".join(line["text_words"] for line in lines)
    ...
    for path in iter_images(benchmark_dir):
        ...
        if words:
            ...
            if lines:
                ...
                _draw_best_sequences(image, lines, target_for_image, top_k=5)
